...
- [ ] **この自動実行環境（Claude Code Remote）から `itunes.apple.com` と `tokyo357.com`（`tokyo357.netlify.app` を含む）への外部通信が遮断されることがある**（2026-08-14 に確認して以降、ほぼ毎回遮断されているが、**間欠的**であることが判明した。プロキシが 403 で CONNECT を拒否。`$HTTPS_PROXY/__agentproxy/status` の `recentRelayFailures` で `connect_rejected` を確認できる）。`tools/ranks.json` の記録を見ると、8/13〜8/18は取得失敗が続いたが、**8/19の回は取得に成功していた**（`checked` が実際に更新され、sukemaru の順位が59位に変わるなど実測値が入っている）。ところが **8/20の回は再び全滅**（8ジャンル中8ジャンルとも `<urlopen error Tunnel connection failed: 403 Forbidden>`、複数回リトライしても回復せず）。つまり「常時遮断」ではなく「その回のセッションによって開通したり遮断されたりする」状態で、いつ開通するか予測できない。`check_ranks.py` は取得失敗時に前回値を保持し `current` を虚偽の「ランク外」で上書きしない実装のままなので、記録が壊れる心配はない。記事執筆は WebSearch（別経路、遮断されていない）が使えるため継続できているが、iTunes lookup APIによる評価数・価格の実測とランキング取得はこのツールでは代替できない。**この状態が続くと、有料アプリの最高順位更新を取り逃している可能性がある。** ネットワークポリシー側で両ホストを常時の許可リストに入れる対応が引き続き最優先。
      ...
      **9/19の回も引き続き全滅**（`itunes.apple.com` 8ジャンル中8ジャンルとも `403 Forbidden`（`check_ranks.py`実行時点）、`WebFetch`で`https://itunes.apple.com/lookup`・`https://tokyo357.com/`を直接試しても`EGRESS_BLOCKED`、`curl`でも`itunes.apple.com`・`tokyo357.netlify.app`・`tokyo357.com`いずれもCONNECTが403で拒否されることを`$HTTPS_PROXY/__agentproxy/status`の`recentRelayFailures`で確認、`api.github.com`は200で正常）。これで**8/20〜9/19と31日連続**の全滅。`checked`は8/19のまま更新できず、`tools/ranks.json`に差分なし（想定どおり）。セッション開始時にdetached HEADを発見（`origin/main`＝`7a6f495`と完全一致、ローカル`main`だけ2コミット遅れ）し、`git checkout -B main`で復旧（作業中の変更を保持したまま、pushは不要、`origin/main`は無傷）。`SKIP_RANK_GUARD=1`で迂回し、live アプリの中で記事の最終更新日が最も古かった（9/11のまま8日更新なし）tsuzukutodoと、次に古く記事数も並んで少なかった（9/12、7本タイ）musuberu/shizukatlからmusuberuを選定して2本執筆。tsuzukutodoはsupport.htmlの既存FAQ「深夜に日付が変わって、その日の記録ができません」（設定の「1日の区切り」を3:00などに変更・0〜5時で選択可）を掘り下げ、境目がずれる理由と区切りを変えた前後の記録の扱いを執筆。musuberuはapp/musuberu.htmlとsupport.htmlの既存FAQ「知っている呼び方で検索しても出てきません」（巻き結び＝インクノット等の別名は検索対象・自分の呼び名を登録すると一覧の表示名も切り替わる）に沿って執筆。いずれも外部情報源は不使用、他社アプリへの言及なし。**この回、netlify.tomlのリダイレクトに/guides記事52本・/appsページ3本ぶんの積み残しがあることを発見した**（9/9〜9/18の複数回のキュー消化で新規記事分のリダイレクトは都度追加されていたはずが、それ以前に書かれた既存記事の多くに`.html`付きURLからのリダイレクトが設定されていなかった。実際にアクセスされた形跡の有無は確認できないが、`.html`付きでインデックスされている可能性があるため、まとめて追加し重複なし・TOML構文有効を確認した）。CSPハッシュは全ページ・`go/*.html`を含む全インラインscript対象で再計算して丸ごと置き換え、268件→272件（JSON-LD 264件＋`go/*.html`8件）になったことを、ページ側から計算したハッシュ集合とnetlify.toml側のハッシュ集合が完全一致（不足0・不要0）することで検証した。sitemap.xmlも`tools/build_sitemap.py`で再生成し、163件とURL数の一致（総ページ数164からSearch Console確認用ファイル1件を除いた数）を確認した。内部リンク全件（存在確認）・JSON-LD全件のjson.loads・非日本語文字混入もすべて検証済み。**非日本語文字混入チェックについて、READMEが指示する`grep -o '[가-힣]\+\|[а-яА-Я]\+' *.html apps/*.html guides/*.html`はこの環境では誤検知する**ことが分かった（`locale`が`LC_CTYPE=POSIX`でUTF-8ロケールが無く、`grep`がバイト単位で比較するため日本語のマルチバイト列を誤ってハングル・キリル文字として拾ってしまう。`C.utf8`ロケールを試しても`Invalid collation character`で失敗する。Pythonの`re`でUnicodeコードポイント範囲（`가-힣`・`Ѐ-ӿ`）を指定して確認したところ混入ゼロ。次回以降もこのgrepコマンドは使わず、Pythonの`re`で確認すること）。4本ある準備中アプリ（だんどりレシピ／ハナログ／オボエル家計簿／ナラセル）の公開状況をWebSearchで再確認したが、今回も実在を検索結果から確認できず（「ナラセル」は今回も無関係の楽器フリマアプリ「NARASELL」がヒットするのみ）、state変更は見送った。**測定手段（順位・評価数）が31日間まったく取得できていない状態が続いている。**状況そのものに変化がなく（新規の異常ではなく同じ遮断が継続しているだけ）、9/18の回の判断を踏襲して`PushNotification`の再試行は見送り、記録の更新のみとした。
      **9/20の回も引き続き全滅**（`itunes.apple.com` 8ジャンル中8ジャンルとも `403 Forbidden`、`https://tokyo357.netlify.app` `https://tokyo357.com` の `/` `/apps/` `/guides/` も `CONNECT tunnel failed, response 403`。`$HTTPS_PROXY/__agentproxy/status` の `recentRelayFailures` でも `itunes.apple.com:443` への `connect_rejected` を確認、`api.github.com` は200で正常）。これで**8/20〜9/20と32日連続**の全滅。`checked` は8/19のまま更新できず、`tools/ranks.json` に差分なし（想定どおり）。セッション開始時に detached HEAD を発見（`origin/main`＝`5a73a73`と完全一致、ローカル `main` だけ4コミット遅れ）し、`git checkout main && git merge --ff-only 5a73a73` で復旧（fast-forwardのみで完結、push不要、`origin/main` は無傷）。`SKIP_RANK_GUARD=1` で迂回し、live アプリの中で記事の last-updated が最も古かった（9/12のまま8日更新なし）shizukatlと、次に古く記事数も並んで9本タイだった（9/13、sukemaruと並び）othelloを選定して2本執筆。shizukatlはsupport.htmlの既存FAQ「投稿の削除やフォロー操作は行われますか」「個人情報は集めていますか」を掘り下げ、Safari拡張機能をインストールする前の不安（「アクセスを許可」が何を許可しているのか）に答える記事を執筆（許可の3段階・対象サイトをx.com/twitter.comの2つに限定していること・通信を行わないことを整理し、既存の動作しない場合のトラブルシュート記事とは異なる「購入前の信頼」の角度で執筆）。othelloはsupport.htmlの既存FAQ「ゲームの進行状況（解放したレベル）が消えました」とapp/othello.htmlの記載（端末内保存・通信なし・サーバー預かりなし）を掘り下げ、機種変更・再インストールで進行状況が消える理由と、消えないもの（App Store購入自体はApple ID紐づきで買い直し不要）を整理して執筆。いずれも外部情報源は不使用、他社アプリへの言及なし。CSPハッシュは全ページ・`go/*.html`を含む全インラインscript対象で再計算して丸ごと置き換え、272件→276件（JSON-LD 268件＋`go/*.html`8件）になったことを、ページ側から計算したハッシュ集合とnetlify.toml側のハッシュ集合が完全一致（不足0・不要0）することで検証した（旧ハッシュのうち1件はguides/index.htmlのItemListスキーマが新規記事2本の追加で内容そのものが変わったことによる想定内の入れ替わりで、他の既存ハッシュはすべて変更前後で一致）。netlify.tomlのリダイレクトも新規記事2本分を追加した。sitemap.xmlも`tools/build_sitemap.py`で再生成し、165件とURL数の一致を確認した。内部リンク全件（存在確認）・JSON-LD全件のjson.loads・非日本語文字混入（Pythonの`re`で確認）もすべて検証済み。3本ある準備中アプリ（だんどりレシピ／ハナログ／オボエル家計簿）についてWebSearchで公開状況を再確認したが、今回も実在を検索結果から確認できず、state変更は見送った。**測定手段（順位・評価数）が32日間まったく取得できていない状態が続いている。**状況そのものに変化がなく（新規の異常ではなく同じ遮断が継続しているだけ）、9/18・9/19の回の判断を踏襲して`PushNotification`の再試行は見送り、記録の更新のみとした。
      **9/21の回も引き続き全滅**（`itunes.apple.com` 8ジャンル中8ジャンルとも `403 Forbidden`、`https://tokyo357.netlify.app` `https://tokyo357.com` の `/` `/apps/` `/guides/` も `CONNECT tunnel failed, response 403`。`$HTTPS_PROXY/__agentproxy/status` の `recentRelayFailures` でも `itunes.apple.com:443` への `connect_rejected` を確認、`api.github.com` は200で正常）。これで**8/20〜9/21と33日連続**の全滅。`checked` は8/19のまま更新できず、`tools/ranks.json` に差分なし（想定どおり）。セッション開始時に detached HEAD を発見。今回は `origin/main` は実際には最新（`a81ee35`、9/20の記事セッションが正しくpushした断面）を指していたが、ローカルの `git branch -vv` 表示上はそれより5コミット古い断面を指していると見えた。`git fetch origin main` すると `origin/main` の参照が実際には更新され、ローカルの参照が単に古いキャッシュだったことが分かった（＝pushは正常に完了していた）。`git checkout main && git merge --ff-only origin/main` で復旧、push不要。`SKIP_RANK_GUARD=1` で迂回し、live アプリの中で記事last-updatedが最も古かった（9/13のまま8日更新なし）sukemaruと、次に古いタイの中で記事数が最少だった（9/14、magedori 7本）magedoriを選定して2本執筆（詳細は`tools/article_queue.md`）。**CSPハッシュを全ページ再計算する際、README記載の`"\n"+m.group(1)+"\n"`という改行付与の手順を、今回使用した正規表現（パターン中に明示的な`\n`を含まない版）にそのまま適用すると、`m.group(1)`自体に前後の改行が既に含まれているため二重に付与してしまい、既存276件のハッシュとほぼ全件不一致になる誤りに気づいた。**`\n`付与は「パターン中の明示的な`\n`が改行を消費してしまう版」の正規表現に対する修正であり、今回の版では不要（`m.group(1)`をそのまま使うのが正しい）。`netlify.toml`に書き込む前に、既存ハッシュとの差分件数が異常（276件中268件不一致）だったことで気づいて修正できたため実害なし。修正後の方式で272 JSON-LD + `go/*.html`8件 = 280件に再計算し、既存276件との差分（1件削除＝guides/index.htmlのItemList更新、5件追加＝新記事4件＋guides/index.html更新後の1件）が想定どおりであることを確認した。**今後CSPハッシュ計算スクリプトを書くときは、使用する正規表現が改行を消費する版かどうかをまず確認し、既存ハッシュとの差分件数が想定より大きい場合は書き込む前に立ち止まること。**sitemap.xmlも`tools/build_sitemap.py`で再生成し、167件（総ページ数167、`404.html`・Search Console確認用ファイルを除く）とURL数の一致を確認した。内部リンク全件（存在確認）・JSON-LD全件のjson.loads・非日本語文字混入（Pythonの`re`で確認）もすべて検証済み。**測定手段（順位・評価数）が33日間まったく取得できていない状態が続いている。**状況そのものに変化がなく、9/18〜9/20の判断を踏襲して`PushNotification`の再試行は見送り、記録の更新のみとした。
      **9/23の回も引き続き全滅**（`check_ranks.py`は8ジャンル中8ジャンルとも`itunes.apple.com`への接続が403 Forbiddenで失敗し、`tools/ranks.json`は無変更（差分なし、`checked`は8/19のまま）。`curl`でも`itunes.apple.com`・`tokyo357.netlify.app`いずれもCONNECTが403で拒否されることを`$HTTPS_PROXY/__agentproxy/status`の`recentRelayFailures`で確認、`api.github.com`は正常）。これで**8/20〜9/23と35日連続**の全滅。セッション開始時、`git status`・`git rev-parse HEAD/main/origin/main`はすべて一致しており、過去に繰り返し再発していたdetached HEAD問題は今回は発生していなかった。前回（9/22）のセッションが`tools/article_queue.md`への記入を怠っていたことに気づき（GUIDESと記事本体だけコミットされていた）、9/22分の2行を遡って記入した。live アプリの中で記事last-updatedが最も古かった（9/15のまま8日更新なし）kigyouojisanと、次に古いタイ（9/16、negotouranai 7本・sagaseru 8本）のうち、negotouranaiは直近の記事群（仕組み・プライバシー・容量・充電・感度調整）でテーマをほぼ使い切っており薄い記事になる懸念があったため、未着手の機能が残っていたsagaseruを選定して2本執筆（詳細は`tools/article_queue.md`）。kigyouojisanはsupport.htmlの既存FAQ「回答の内容は信用してよいですか」とapp/kigyouojisan.htmlの「根拠が見える」機能を掘り下げ、回答の下に表示される参照知識の読み方を執筆。sagaseruはsupport.htmlの既存FAQ「文字起こしが始まりません」を掘り下げ、iOS音声認識設定・アプリ内自動文字起こし設定・認識モデル初回ダウンロードの3点を執筆。いずれも外部情報源は不使用、他社アプリへの言及なし。CSPハッシュは全ページ・`go/*.html`を含む全インラインscript対象で再計算して丸ごと置き換え、284件→280件（JSON-LD 272件＋`go/*.html`8件）になったことを、ページ側から計算したハッシュ集合とnetlify.toml側のハッシュ集合が完全一致（不足0・不要0）することで検証した（284件中9件が今回の再計算で不要と判明し除外、5件が新規追加。除外分は過去の増分追記だけで一度も全体を洗い直していなかったための残存ハッシュと見られ、実害はなかった）。netlify.tomlのリダイレクトは新規記事2本分を追加、TOML構文は`tomllib`でパース確認。sitemap.xmlも`tools/build_sitemap.py`で再生成し、171件（総ページ数179から`404.html`・Search Console確認用ファイル・`go/*.html`8件を除いた171件）とURL数の一致を確認した。内部リンク全件（存在確認、`/t357`除く）・JSON-LD全件280件のjson.loads・非日本語文字混入（Pythonの`re`で確認）もすべて検証済み。ローカルサーバーで主要ページ・新規記事2本とも200を確認（本番URLは前述のとおり到達不可のため未確認）。4本ある準備中アプリ（だんどりレシピ／ハナログ／オボエル家計簿／ナラセル）の公開状況をWebSearchで再確認したが、今回も実在を確認できず（4アプリID直接検索・名称検索とも一致なし）、state変更は見送った。**測定手段（順位・評価数）が35日間まったく取得できていない状態が続いている。**状況そのものに変化がなく、9/18〜9/22の判断を踏襲し、記録の更新のみとした（`PushNotification`は過去2回`InputValidationError`で失敗しており、状況に変化がないため今回は試行を見送った）。
      **9/22の回も引き続き全滅**（セッション開始時に`$HTTPS_PROXY/__agentproxy/status`の`recentRelayFailures`が空だったため一瞬開通したかと期待したが、`check_ranks.py`実行直後に確認すると`itunes.apple.com:443`への`connect_rejected`が20件連続で記録されており、8ジャンル中8ジャンルとも`403 Forbidden`。`https://tokyo357.netlify.app`の`/` `/apps/` `/guides/`も`CONNECT tunnel failed, response 403`）。これで**8/20〜9/22と34日連続**の全滅。`checked`は8/19のまま更新できず、`tools/ranks.json`に差分なし（想定どおり）。セッション開始時に detached HEAD を発見（`origin/main`＝`23eaf28`と完全一致、ローカル`main`だけ12コミット遅れ）し、`git checkout -B main`で復旧（作業中の変更を保持したまま、pushは不要、`origin/main`は無傷）。`SKIP_RANK_GUARD=1`で迂回し、live アプリの中で記事の last-updated が最も古かった（9/14のまま8日更新なし）kaeseruと、次に古いタイの中で記事数が最少だった（9/15、kagikake 7本、kigyouojisanは10本のため除外）kagikakeを選定して2本執筆。kaeseruは既存記事になかった「返済負担率」を新規テーマに選び、WebSearchでフラット35公式の基準（年収400万円未満は総返済負担率30%以下、400万円以上は35%以下）を確認したうえで、「借りられる上限」と「無理なく返せる額」が別の数字であることを、app/kaeseru.htmlに実装済みの借入可能額の逆算機能・金利上昇シナリオ試算に絡めて執筆（特定の他社アプリへの言及なし、民間金融機関は基準非公開である旨も明記）。kagikakeはapp/kagikake.htmlに実装済みの「ダミーのパスコード」機能を、人にスマホを見せてと言われる場面という具体的な困りごとに結びつけて執筆（外部情報源は不使用、他社アプリへの言及なし）。CSPハッシュ再計算で、これまでの計算スクリプトが`go/*.html`の`type`属性なしの`<script>`タグ（8件）を対象外にしたまま`go/*.html`を走査していたため、JSON-LD分の276件のみを新ハッシュとして書き込みそうになった（既存280件との差分が「削除1件・追加5件」ではなく「削除5件・追加1件」規模になり、9/21の教訓どおり書き込み前に差分件数の異常で気づいた）。`go/*.html`用に`type`属性なしの`<script>(.*?)</script>`パターンを別に用意して284件（JSON-LD 276件＋go 8件）に修正し、既存280件との差分が削除1件（guides/index.htmlのItemList更新による想定内の入れ替わり、実際にguides/index.htmlから再計算した値と一致することを確認）・追加5件（新記事2本×2件＋上記ItemList更新分）であることを検証してから書き込んだ。netlify.tomlのリダイレクトは新規記事2本分のみが不足しており（積み残しなし）追加、重複なし・TOML構文有効（`tomllib`でパース確認）を確認。sitemap.xmlも`tools/build_sitemap.py`で再生成し、169件（総ページ数179から`404.html`・Search Console確認用ファイル・`go/*.html`8件を除いた169件）とURL数の一致を確認した。内部リンク全件6,929件（存在確認、`/t357`除く）・JSON-LD全件276件のjson.loads・非日本語文字混入（Pythonの`re`で確認）もすべて検証済み。ローカルサーバーで主要ページ・新規記事2本とも200を確認（本番URLは前述のとおり到達不可のため未確認）。4本ある準備中アプリ（だんどりレシピ／ハナログ／オボエル家計簿／ナラセル）の公開状況をWebSearchで再確認したが、今回も実在を検索結果から確認できず、state変更は見送った。**測定手段（順位・評価数）が34日間まったく取得できていない状態が続いている。**状況そのものに変化がなく、9/18〜9/21の判断を踏襲して`PushNotification`の再試行は見送り、記録の更新のみとした。
      **9/24の回も引き続き全滅**（`check_ranks.py`は8ジャンル中8ジャンルとも`itunes.apple.com`への接続が403 Forbiddenで失敗し、`tools/ranks.json`は無変更（差分なし、`checked`は8/19のまま）。`curl`でも`tokyo357.netlify.app`の`/` `/apps/` `/guides/`がすべてCONNECTで403拒否されることを`$HTTPS_PROXY/__agentproxy/status`の`recentRelayFailures`で確認（`itunes.apple.com:443`への`connect_rejected`多数）。これで**8/20〜9/24と36日連続**の全滅。セッション開始時、`git status`は`On branch main`・`origin/main`と一致しておりdetached HEADの再発なし。記事本数が最少タイの live アプリから、negotouranai（7本、仕組み・プライバシー・容量・充電・感度調整の既存テーマとは別角度で「いびき・寝息も録れてしまう」を選定）とkagikake（8本、「大きな動画でも落ちない」という既存機能説明を掘り下げ「動画でアプリが落ちる理由」を選定）の2本を執筆。いずれもapp/*.htmlに実装済みの機能説明を根拠とし、外部情報源は不使用、他社アプリへの言及なし。医療にかかる「いびき」の話は対策を断定せず医療機関への相談を明記。CSPハッシュは全ページ・`go/*.html`を含む全インラインscript対象で再計算して丸ごと置き換え、280件→284件（新規記事4件＋guides/index.html更新分1件を追加、過去の増分追記で残っていた不要ハッシュ1件を除外）になったことを、ページ側から計算したハッシュ集合とnetlify.toml側のハッシュ集合が完全一致（不足0）することで検証した。netlify.tomlのリダイレクトは新規記事2本分を追加。sitemap.xmlも`tools/build_sitemap.py`で再生成し、173件とスクリプトの報告件数が一致することを確認した。内部リンク全件（`href`/`src`、`/t357`除く）・JSON-LD全件284件のjson.loads・非日本語文字混入（`LC_ALL=C.utf8`でロケールを明示、既定のPOSIXロケールだとマルチバイト文字がハングル/キリル文字として誤検出される点に注意）もすべて検証済み・異常なし。ローカルサーバーで主要ページ・新規記事2本とも200を確認（本番URLは前述のとおり到達不可のため未確認）。4本ある準備中アプリ（だんどりレシピ／ハナログ／オボエル家計簿／ナラセル）はtools/apps.pyのstateを確認したが、iTunes lookup APIが遮断されており公開状況を外部から確認する手段がないため、state変更は見送った。**測定手段（順位・評価数）が36日間まったく取得できていない状態が続いている。**状況そのものに変化がなく、`PushNotification`は9/15・9/17に`InputValidationError`で失敗して以降、状況に変化がないため試行を見送ってきたが、遮断が36日（1か月超）に達し長期化が明白になったため、今回は試行したところ成功した（9/12以来の成功、9/15・9/17のInputValidationErrorは再現しなかった）。
