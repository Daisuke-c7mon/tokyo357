...
- [ ] **この自動実行環境（Claude Code Remote）から `itunes.apple.com` と `tokyo357.com`（`tokyo357.netlify.app` を含む）への外部通信が遮断されている**（2026-08-14 確認、2026-08-16・2026-08-17・2026-08-18 も再確認・継続中。プロキシが 403 で CONNECT を拒否）。`tools/check_ranks.py` がランキングを取得できず、ページの生死確認・評価数の実測もできない状態が続いている。ネットワークポリシーの許可リストに両ホストを追加する必要がある。`check_ranks.py` 側は取得失敗時に前回値を保持し `current` を虚偽の「ランク外」で上書きしない実装に修正済み（2026-08-14）。**ランキング記録は2026-08-12を最後に6日分（8/13〜8/18）欠落したまま。** ネットワークポリシー側の対応が最優先。**この状態が続くと、6本の有料アプリの最高順位更新を6日間ずっと取り逃している可能性がある。**
- [ ] **毎回の作業開始時に `git status` / `git branch` で `main` ブランチにいることを確認する。**
      ...
      **2026-08-18 の実行でも同じパターンが再発**（`HEAD` が `main` から5コミット分 detached。
      8/17 セッション自身が作った成果を含む）。3回連続の再発により、**このリモート実行環境が
      セッション開始時にブランチではなく特定コミットへdetached HEADでチェックアウトする傾向がある**
      ことはほぼ確実と見てよい。いずれの回も fast-forward で復旧・pushできたため実害はなかったが、
      **もしいつか fast-forward できない形（コミットが分岐する等）で発生した場合、
      過去の成果を失うリスクがある。**
      **作業開始直後に必ず `git checkout main` してから始める運用は今後も継続すること。**
      3回連続の再発を踏まえ、環境のセッション起動処理（リポジトリのcheckout方法）側の
      調査を強く推奨する。
